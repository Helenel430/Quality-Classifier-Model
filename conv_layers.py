...
# Make training (70%) testing datasets (10%)
...
# Initalize List of Dict for training and testing
image_map_train = []
image_map_valid = []
image_map_test = []
...
class NeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Convolutional layers take [batch size, channels, image size]
        x = x.transpose(3, 1)
        x = x.float()
...
scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=5, eta_min=1e-2)
...
for epoch in range(epochs):
    ...
    # Log change to learning rate in tensorboard
    for i, param_group in enumerate(optimizer.param_groups):
        writer.add_scalar(f"Learning Rate/ Group {i}", param_group["lr"], current_epoch)
    writer.flush()
